 ...
 * 且A上存在元注解B，B上存在元注解C，则对注解A进行解析，将得到包含根注解A，
 * 以及其元注解B、C在内的合成元注解聚合{@link GenericSynthesizedAggregateAnnotation}。
 *
 ...
 *
 * <p>当扫描的注解对象经过{@link SynthesizedAnnotationSelector}筛选后，
 * 将会被转为{@link MetaAnnotation}，并使用在实例化时指定的{@link SynthesizedAnnotationPostProcessor}进行后置处理。<br>
 * 默认情况下，将注册以下后置处理器以对{@link Link}和其扩展注解提供支持：
 * <ul>
 ...
 * 返回的属性值将根据合成注解中对应原始注解属性上的{@link Link}注解而有所变化。
