...
public abstract class AbstractHierarchyScanner<T extends AbstractHierarchyScanner<T>> implements AnnotationScanner {
	...
	public void scan(AnnotationProcessor processor, AnnotatedElement annotatedEle, Predicate<Annotation> filter) {
		scanTypeHierarchy(processor, annotatedEle, filter);
	}
	...
	private void scanTypeHierarchy(
		AnnotationProcessor processor, AnnotatedElement annotatedEle, Predicate<Annotation> filter) {
		...
		while (!classDeque.isEmpty()) {
			...
			for (final Class<?> targetClass : currClassQueue) {
				...
				// 处理层级索引和注解
				final List<Annotation> targetAnnotations = Stream.of(getAnnotationsFromTargetClass(annotatedEle, verticalIndex, targetClass))
				...
				for (final Annotation annotation : targetAnnotations) {
					horizontalIndex++;
					processor.process(verticalIndex, horizontalIndex, annotation);
